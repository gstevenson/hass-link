...
{
    private readonly List<(string InstanceName, PerformanceCounter Recv, PerformanceCounter Send)> _adapters = [];
    ...
    public NetworkSensor()
    {
        try
        {
            ...
            foreach (var instance in instances)
            {
                ...
                _adapters.Add((instance, recv, send));
            }
        }
        catch
        {
    ...
    public Task<IReadOnlyList<SensorReading>> GetReadingsAsync()
    {
        ...
        foreach (var (instance, recv, send) in _adapters)
        {
            ...
            readings.Add(new($"net_{safeId}_recv", $"Network {instance} Download", recvMbps.ToString("F3"), "MB/s", null, "mdi:download-network"));
            readings.Add(new($"net_{safeId}_send", $"Network {instance} Upload", sendMbps.ToString("F3"), "MB/s", null, "mdi:upload-network"));
        }
    ...
    public void Dispose()
    {
        foreach (var (_, recv, send) in _adapters)
        {
            recv.Dispose();
            send.Dispose();
        }
